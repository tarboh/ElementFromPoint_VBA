...
#If Win64 Then
    Public Const pIUnknownRelease As Long = 8   'Address of 2nd Function in the virtual function table of the IUIAutomationElement Interface : (2-1)th * 8 Byte'
    Public Const pElementFromPoint As Long = 56 'Address of 8th Function in the virtual function table of the CUIAutomation Class            : (8-1)th * 8 Byte'
    Public Const pCount = 2                     'Number of arguments in 64bit environment ( pt, Element* )'
#Else
    Public Const pIUnknownRelease As Long = 4   'Address of 2nd Function in the virtual function table of the IUIAutomationElement Interface : (2-1)th * 4 Byte'
    Public Const pElementFromPoint As Long = 28 'Address of 8th Function in the virtual function table of the CUIAutomation Class            : (8-1)th * 4 Byte'
    Public Const pCount = 3                     'Number of arguments in 32bit environment ( pt.x, pt.y, Element* )'
#End If
...
Public Function ElementFromPoint(ByRef uia As CUIAutomation, ByRef pt As POINTAPI) As IUIAutomationElement
    ...
    #If Win64 Then
        ...
        'これを防ぐため、事前にPINTAPIの各メンバを、呼び出し先で格納されるtagPOINTと互換性がある単一の変数に手動で格納しておく必要がある。'
    ...
    If lRtn = S_OK Then
        If vRtn = S_OK Then
            ...
            If Not Element Is Nothing Then
                '内部用の変数であるIUIAutomationElementインターフェースを、IUnknownReleaseメソッドで解放する'
                uRtn = DispCallFunc(ObjPtr(Element), pIUnknownRelease, CC_STDCALL, vbLong, 0, 0, 0, vRtn)
                If uRtn <> S_OK Then
                    SetLastError uRtn
